...
export class HuggingFaceProvider implements AIProvider {
  public readonly id: AIProviderId = 'huggingface';
  // Use the standard OpenAI compatible chat completions endpoint on Inference API
  private endpoint = 'https://api-inference.huggingface.co/v1/chat/completions';

  isConfigured(): boolean {
    return Boolean(process.env.HUGGINGFACE_API_KEY) && this.getConfiguredModels().length > 0;
  }

  getConfiguredModels(): string[] {
    return parseModelList(process.env.HUGGINGFACE_MODELS);
  }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 401 || response.status === 403) {
          ...
          errorCode = 'AI_PROVIDER_FAILED'; // Model is loading or unavailable
        }

        throw new AIProviderError({
          code: errorCode,
          message: `Hugging Face error: ${response.statusText}`,
          retryable,
          provider: this.id,
          model,
      ...
      if (!markdown) {
        throw new AIProviderError({
          ...
          message: 'Received empty response from Hugging Face',
          retryable: true,
          provider: this.id,
          model
        });
      ...
      throw new AIProviderError({
        ...
        message: error.message || 'Unknown Hugging Face error',
        retryable: true,
        provider: this.id,
        model,
        cause: error
  ...
  async generateImprovement(
    ...
    const apiKey = process.env.HUGGINGFACE_API_KEY;
    const realModel = model || this.getConfiguredModels()[0] || 'Qwen/Qwen2.5-Coder-32B-Instruct';
